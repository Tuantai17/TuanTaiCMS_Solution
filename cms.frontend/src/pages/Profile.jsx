          ...
          <div className="profile-sidebar-card">
            <div className="profile-sidebar-header">
              <div className="profile-avatar-wrapper">
                ...
                <button className="profile-avatar-edit-btn" onClick={() => avatarInputRef.current?.click()} title="Đổi ảnh đại diện">
                  <i className="fa-solid fa-camera"></i>
                </button>
              </div>
              <h4 className="profile-sidebar-name">{fullName}</h4>
              <p className="profile-sidebar-email">{email}</p>
              <span className="profile-member-badge"><i className="fa-solid fa-crown"></i> Thành viên</span>
            </div>

            <ul className="profile-sidebar-menu">
              {MENU_ITEMS.map((item, idx) => (
                <React.Fragment key={item.key}>
                  {item.isLogout && <li><div className="profile-menu-divider"></div></li>}
                  <li>
                    <button
                      type="button"
                      className={`profile-menu-item ${activeTab === item.key ? 'active' : ''} ${item.isLogout ? 'logout-item' : ''}`}
